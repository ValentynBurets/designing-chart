...
export default function AdminTaskInfoPage(){
    ...
    const [stateTime, setStateTime] = useState('11:00');
    ...
    const getExercise = async() =>{
        axios
        .get(`https://localhost:44383/api/Exercises/GetById`,{
            params: {
                id: exerciseId,
            }
        })
        .then((responce) => {
            ...
            if (data != null) {
                ...
                setStateDesc(data.description);
                let diagramElement = document.getElementById('diagram');
                let diagram = diagramElement.ej2_instances[0];
                diagram.loadDiagram(data.etalonChart);
                setStateDate(data.expirationDate.split('T')[0])
                setStateTime(data.expirationDate.split('T')[1])
                setStateCategory(data.categoryName)
                setStateMaxMark(data.maxMark)
            }
        })
        .catch((e) => {
